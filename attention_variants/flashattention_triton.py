...
def flash_attention_kernel( q_ptr: torch.Tensor, 
    ...
    for i in tl.range(0, T_r):
        ...
        o_i = tl.load(o_ptrs)   # o_i   has shape [B_r, d]

        ## 5: on-chip, initialize o_prev_i, l_prev_i, m_prev_i
        o_prev = tl.zeros((B_r, d), dtype=tl.float16)             # 5: o(0)_i has 0s    of shape [B_r, d]
        l_prev = tl.zeros((B_r,),    dtype=tl.float16)             # 5: l(0)_i has 0s    of shape [B_r]
        m_prev = tl.zeros((B_r, 1), dtype=tl.float16) - math.inf  # 5: m(0)_i has -infs of shape [B_r]


        k_ptrs = k_ptr + off_c[:, None] * stride_kn + off_d[None, :] * stride_kd
        v_ptrs = v_ptr + off_c[:, None] * stride_kn + off_d[None, :] * stride_kd
        
        # online softmax
        ## 6: for 1 <= j <= T_c do
        for j in tl.range(0, T_c):
            k_j = tl.load(k_ptrs)  ## 7: load K_j from HBM to on-chip SRAM. K_j has shape [B_c, d]
            v_j = tl.load(v_ptrs)  ## 7: load V_j from HBM to on-chip SRAM. V_j has shape [B_c, d]
        ...
        tl.store(o + off_r, o_prev / l_prev)          ## 11: On chip, compute O_i = O(j)_i / l(j)_i
        tl.store(l + off_r, m_prev + tl.log(l_prev))  ## 12: On chip, compute L_i = 
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
    ...
    grid = lambda META: (triton.cdiv(M, META['B_r']) * triton.cdiv(N, META['B_c']), )
